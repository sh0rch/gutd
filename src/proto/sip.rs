...
pub fn update_cached_date() {
    ...
    let packed =
        (year as u32) | ((month as u32) << 16) | ((day as u32) << 20) | ((weekday as u32) << 25);
...
pub fn parse_timestamp_from_date_header(buf: &[u8]) -> Option<u64> {
    ...
    let date_pos = buf
        .windows(date_marker.len())
        .position(|w| w == date_marker)?;
...
fn write_options_keepalive(
    ...
    write!(
        ...
        dport, domain, dport, src_ip, sport
    )
    .unwrap();
...
pub fn write_header(
    ...
    let to_tag_val = crate::proto::feistel::sip_hash32(
    ...
    match kind {
        ...
        SipKind::Response401 => {
            ...
            let nonce_val =
                crate::proto::feistel::sip_hash32((timestamp_us / 1000) as u32, feistel_key);
            write!(
                cursor,
    ...
    if matches!(
        kind,
        SipKind::Response200 | SipKind::Response401 | SipKind::Response403
    ) {
